...
def load_dataset(root_path="datasets", dataset="english", batch_size=8, block_size=128):
    in_path = op.join(root_path, dataset)
    files = glob.glob(op.join(in_path, "*.npy"))
    if len(files) == 0:
        raise ValueError(f"No files found in dataset {in_path}")
    tokens = np.hstack([np.load(f) for f in files])
